...
> {
   ...
   *
   * // Only select the `id`
   * const userWithIdOnly = await db.user.findMany({ select: { id: true } })
   *
   */
   ...
   * @example
   * // Create many Users
   * const user = await db.user.createManyAndReturn({
   *   data: [
   *     // ... provide data here
   *   ]
   * })
   ...
   * const userWithIdOnly = await db.user.createManyAndReturn({
   *   select: { id: true },
   *   data: [
   *     // ... provide data here
   *   ]
   ...
   * const userWithIdOnly = await db.user.updateManyAndReturn({
   *   select: { id: true },
   *   where: {
   *     // ... provide filter here
   *   },
   ...
   * // Where email contains db.io
   * // Limited to the 10 users
   * const aggregations = await db.user.aggregate({
   *   _avg: {
   *     age: true,
   *   },
   *   where: {
